...
from Api.Input import Input  # 검색 입력 필드를 위해 Input 클래스를 가져옵니다.
...
class BasicWordbookView:
    def __init__(self):
        ...
        star_filled_image_path = os.path.join(current_dir, "../Images/star_filled.png")
        star_empty_image_path = os.path.join(current_dir, "../Images/star_empty.png")
        home_image_path = os.path.join(current_dir, "../Images/home.png")
    ...
    def display_page(self, page, word_data, total_pages, favorite_status, checkboxes):
        ...
        for i, word in enumerate(word_data):
            ...
            meaning_text = self.font_small.render(word['mean'], True, pygame.Color('black')) if len(
                word['mean']) > 5 else self.font_large.render(word['mean'], True, pygame.Color('black'))
