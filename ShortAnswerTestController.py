    ...
    def run(self):
        while self.running:
                ...
                if event.type == pygame.USEREVENT and not self.enter_event_handled:
                    if event.name == 'enterEvent' and not (self.selecting_mode or self.selecting_levels):
                        ...
                        if user_input == correct_answer:
                            ...
                            print("오답!")
                            self.model.save_wrong_answer(self.user_id, self.words[self.current_word_index].word)  # 틀린 답 저장
            ...
            if self.selecting_mode:
                ...
                if self.current_word_index < len(self.words):
                    self.view.render_word(self.game_mode, self.words[self.current_word_index], '주관식' in self.game_mode, self.current_word_index, len(self.words))
                else:
