...
class WordCardController:
    ...
    def display_word(self):
        if self.wordcards:
            ...
            self.view.display_word(current_wordcard.word, self.current_card_index, len(self.wordcards),
                                   current_wordcard.is_favorite)
        else:
    ...
    def show_meaning(self):
        ...
        if self.showing_meaning:
            if self.wordcards:
                current_wordcard = self.wordcards[self.current_card_index]
                self.view.display_meaning(current_wordcard.word, current_wordcard.mean, self.current_card_index,
                                          len(self.wordcards), current_wordcard.is_favorite)
        else:
            self.display_word()
    ...
    def filter_wordcards(self):
        self.wordcards = self.model.fetch_wordcards(self.selected_levels, id=self.id,
                                                    only_favorites=self.show_favorites_only,
                                                    only_wrong=self.show_wrong_only)
        self.current_card_index = 0
        self.display_word()
    ...
    def search_word(self):
        ...
        if word:
            self.wordcards = self.model.search_wordcards(word, self.id, self.selected_levels, self.show_favorites_only,
                                                         self.show_wrong_only)
            self.current_card_index = 0
            self.display_word()
        else:
            self.filter_wordcards()
