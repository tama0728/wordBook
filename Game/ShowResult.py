...
class ShowResult:
    def __init__(self, screen, screen_width, screen_height):
        ...
        self.update_score = Check_Write_Ranking()

        pygame.font.init()
        self.font = pygame.font.SysFont("d2coding", 36)

    def show_number_result(self, number, username):
        self.update_score.update_user_rain_score(username, number)
        self.update_score.update_rain_ranking(username, number)
        result_text = "Result: {}".format(number)
        return self.show_result(result_text)

    def show_time_result(self, elapsed_time, username):
        self.update_score.update_user_card_score(username, elapsed_time)
        self.update_score.update_card_ranking(username, elapsed_time)
